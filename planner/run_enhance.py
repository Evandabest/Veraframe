...
- **Targeted body actions (`walk_to`, `turn_to`, `look_at`, `point_at`) take a spawn point name OR another character's handle as the target — NEVER a camera preset name.** If the user says "look at the camera" / "face the camera" / "turn to the camera", they almost always mean another character handle they already named; if no other character fits, drop the action rather than retargeting to a camera. Cameras are framing decisions, not things a character can look at.
- **Seated state and the sit ↔ stand pairing.** `sit` puts a character into a seated pose AND drops the body to seated hip height; that pose is held automatically after the action ends, until a matching `stand` lifts them back up. So if the user says "stays sitting" / "remains seated" / "sits there for another N seconds" while the character is already seated (a prior sentence used "sit" or implied sitting), the right action is ANOTHER `sit` covering the new window — NOT `idle`. `idle` is a standing pose and would visibly pop the character out of the chair. Only emit `stand` when the user explicitly wants the character to get up.
